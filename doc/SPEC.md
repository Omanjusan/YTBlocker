...
│   ├── card-buttons.ts   動画カードへのボタン注入UI
...
content.ts ─┬─> blocker.ts ─┬─> shared/storage.ts ─> shared/types.ts
            │               └─> toast.ts
            ├─> card-buttons.ts ─> blocker.ts, shared/debug.ts
...
ブロック判定・適用のコアロジック。card-buttons / menu-injector 両方から利用される共通基盤。
...
| `blockAndLog` | `(card, target, value, title, channel, onAdded) => Promise<void>` | ルール登録・ログ保存・カード除去・トースト表示までを一括実行。card-buttonsとmenu-injectorの両方から呼ばれる共通処理 |
...
## content/card-buttons.ts

動画カードに直接「🚫 動画」「🚫 CH」ボタンを差し込むUI。

- `INJECTED_ATTR = 'data-ytb'`: 二重挿入防止用の属性マーカー

| 関数 | シグネチャ | 説明 |
|---|---|---|
| `makeBtn` | `(label, color, onClick) => HTMLButtonElement` | カード内に差し込むブロックボタン1個分のDOM要素を生成 |
| `injectCardButtons` | `(card: Element, onAdded: OnAdded) => void` | 1枚の動画カードにボタンを差し込む。`INJECTED_ATTR`が既についていれば二重挿入を避けてスキップ |
| `injectAllCardButtons` | `(onAdded: OnAdded) => void` | 現在DOM上にある全カードを走査し、それぞれにブロックボタンを差し込む |
...
| `injectButtons` | `() => void` | カードへのブロックボタン注入をやり直す。ボタン押下時は`refresh()`経由で再適用する |
| `refresh` | `() => Promise<void>` | storageからルール/設定を読み直し、ブロック適用とボタン注入をやり直す |
...
- `card-buttons.ts` の `ytm-shorts-lockup-view-model-v2` セレクタは検索結果ページ用の対応漏れを追加で修正したもの
